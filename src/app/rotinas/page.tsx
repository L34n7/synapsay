...
export default function RoutinesPage() {
  ...
  async function load() {
    ...
    setLoading(false);
    if (!response.ok) {
      setError(data.error ?? "Falha ao carregar rotinas.");
      return;
    }
      ...
      <div className={styles.shell}>
        <header className={styles.header}>
          <div>
            ...
            <p className={styles.subtitle}>
              Configure ações recorrentes por horário e contexto. Você também pode criar,
              alterar ou pausar rotinas conversando naturalmente com a Synapsay.
            </p>
          </div>
        ...
        <section className={styles.panel}>
          <div className={styles.panelHeader}>
            <div>
              <h2 className={styles.panelTitle}>Nova rotina</h2>
              <span className={styles.panelHint}>Defina quando e como o assistente deve agir.</span>
            </div>
          </div>
          ...
          <form onSubmit={createRoutine} className={styles.form}>
            ...
            <label className={`${styles.field} ${styles.span4}`}>
              ...
              <select
                ...
                onChange={(event) => setForm({ ...form, confirmation_mode: event.target.value })}
              >
            ...
            <div className={styles.optionsBox}>
              <label className={styles.option}>
                <input
                  ...
                  onChange={(event) => setForm({ ...form, sourcesOnly: event.target.checked })}
                />
                <span>
                  <strong>Usar somente os sites informados</strong>
                  <small>Quando ativado, o briefing não utiliza fontes externas à lista.</small>
                </span>
              </label>
              ...
              <label className={styles.option}>
                ...
                <span>
                  ...
                  <small>A Synapsay poderá sugerir mudanças de duração, assunto ou fonte.</small>
                </span>
              </label>
            </div>
            ...
            <div className={styles.actions}>
              <button disabled={saving} className={styles.primaryButton}>
                {saving ? "CRIANDO ROTINA..." : "CRIAR ROTINA"}
              </button>
            </div>
          </form>
        </section>
        ...
        <section className={styles.listSection}>
          ...
          {loading ? (
            ...
            <div className={styles.empty}>
              Nenhuma rotina criada ainda. Você pode usar o formulário acima ou pedir pelo assistente.
            </div>
          ) : (
                ...
                <article key={routine.id} className={styles.card}>
                  <div>
                    ...
                    <div className={styles.meta}>
                      ...
                      <span>
                        {routine.ends_on ? `Até ${routine.ends_on}` : "Sem data final"}
                      </span>
                    ...
                    <div className={styles.meta}>
                      <span>
                        {routine.adapt_from_memories
                          ? "Personaliza com memórias"
                          : "Sem adaptação por memórias"}
                      </span>
                      <span>
                        {routine.suggest_adjustments
                          ? `Feedback a cada ${routine.feedback_interval} execução(ões)`
                          : "Sem pedido de feedback"}
                      </span>
                    </div>
                  </div>
