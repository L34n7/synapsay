...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase.from("assistant_routines").insert({ ...payload, user_id: userId }).select("*").single();
    if (error) throw error;
    return NextResponse.json({ routine: data }, { status: 201 });
  } catch (reason) {
    return NextResponse.json({ error: reason instanceof Error ? reason.message : "Não foi possível criar a rotina." }, { status: 400 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase.from("assistant_routines").update(payload).eq("id", id).eq("user_id", userId).select("*").single();
    if (error) throw error;
    return NextResponse.json({ routine: data });
  } catch (reason) {
    return NextResponse.json({ error: reason instanceof Error ? reason.message : "Não foi possível atualizar a rotina." }, { status: 400 });
  }
}
