...
function buildPrompt(routine:any, memories:any[], feedback:any[]) {
  ...
  const memoryContext=routine.adapt_from_memories && memories.length
    ? `Preferências e interesses aprovados do usuário, para priorização leve sem alterar a rotina: ${memories.map((m:any)=>m.content).join(" | ")}`
    : "Não personalize por memória nesta execução.";
  const feedbackContext=feedback.length
    ? `Feedback recente do usuário sobre esta rotina: ${feedback.map((f:any)=>f.message).join(" | ")}`
  ...
  return [
    ...
    sourceRule,memoryContext,feedbackContext,
    "As memórias apenas ajustam a prioridade. Não inclua assuntos sensíveis ou irrelevantes só porque aparecem numa memória.",
    ...
    "Comece diretamente pelo resumo, sem explicar como pesquisou. Não invente fatos e diferencie informação confirmada de notícia em desenvolvimento.",
...
export async function POST(request:Request){
  ...
  if(run?.status==="awaiting_confirmation")return NextResponse.json({error:"Esta rotina ainda aguarda confirmação."},{status:409});
  if(run?.expires_at&&new Date(run.expires_at).getTime()<Date.now())return NextResponse.json({error:"A oportunidade desta rotina expirou."},{status:410});
  ...
  try{
    const response=await fetch("https://api.openai.com/v1/responses",{
      ...
      body:JSON.stringify({model:"gpt-5-mini",tools:routine.action_type==="news_briefing"?[{type:"web_search"}]:[],input:buildPrompt(routine,memories??[],feedback??[])}),
    });
    ...
    return NextResponse.json({content,sources,cached:false,askFeedback:Boolean(routine.suggest_adjustments&&((routine.execution_count+1)%Math.max(1,routine.feedback_interval||3)===0))});
  }catch(reason){
