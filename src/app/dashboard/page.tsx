...
export default function Dashboard() {
  ...
  const scheduleVoiceSelectionTimeout = useCallback(() => {
    if (voiceSelectionTimeoutRef.current) {
      window.clearTimeout(voiceSelectionTimeoutRef.current);
    }
    voiceSelectionTimeoutRef.current = window.setTimeout(() => {
      voiceSelectionTimeoutRef.current = null;
      setTranscript("Escolha de voz expirada. Quando quiser, é só pedir para trocar minha voz.");
    }, 90_000);
  }, []);

  const executePersonalityAssistant = useCallback(
    async (call: RealtimeFunctionCall) => {
      const channel = dataChannelRef.current;
      if (
        !channel ||
      ...
      try {
        ...
        if (!response.ok) {
          output = {
            ...
            error:
              (output as { error?: string })?.error ??
              "Não foi possível ajustar a voz agora.",
          };
        }
      } catch {
        output = {
          status: "error",
          error: "Não foi possível ajustar a voz agora.",
        };
      }

      const result = output as VoicePreferenceOutput;
      if (result.previewVoice) {
        applyRealtimeVoice(result.previewVoice);
        scheduleVoiceSelectionTimeout();
      } else if (result.currentVoice) {
        applyRealtimeVoice(result.currentVoice);
    ...
    [applyRealtimeVoice, scheduleVoiceSelectionTimeout],
  );
  ...
  return (
    ...
    >
      ...
      <aside className={styles.rail} aria-label="Navegação principal">
        ...
        <span />
        <a href="/personalidade" aria-label="Configurações"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M19 12a7 7 0 0 0-.1-1l2-1.6-2-3.4-2.5 1a8 8 0 0 0-1.7-1L14.3 3h-4.6l-.4 3a8 8 0 0 0-1.7 1L5 6 3 9.4 5.1 11a7 7 0 0 0 0 2L3 14.6 5 18l2.6-1a8 8 0 0 0 1.7 1l.4 3h4.6l.4-3a8 8 0 0 0 1.7-1l2.6 1 2-3.4-2.1-1.6a7 7 0 0 0 .1-1Z"/></svg></a>
      </aside>
